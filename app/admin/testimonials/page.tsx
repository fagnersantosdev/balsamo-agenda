...
export default function AdminTestimonialsPage() {
  ...
  function renderList(list: AdminTestimonial[]) {
    ...
    if (list.length === 0) return (
      <div className="text-center py-12 border-2 border-dashed border-[#97709B]/10 rounded-2xl">
        <MessageSquare className="mx-auto w-10 h-10 text-[#97709B]/20 mb-3" />
        <p className="text-sm font-medium text-[#97709B]/60">Nenhum depoimento encontrado.</p>
      </div>
    );
      ...
      <div className="grid grid-cols-1 gap-4">
        {list.map((t) => (
          <motion.div
            ...
            className="bg-white rounded-2xl border border-[#97709B]/15 p-5 shadow-sm hover:shadow-md transition-shadow"
          >
            <div className="flex justify-between items-start gap-4 mb-4">
              <div className="flex gap-3 items-center">
                <div className="w-10 h-10 rounded-full bg-[#97709B]/10 flex items-center justify-center text-[#97709B] font-bold">
                  {(t.author || "A")[0].toUpperCase()}
                </div>
                <div>
                  <h4 className="font-bold text-[#083536]">{t.author || "Anônimo"}</h4>
                  <div className="flex items-center gap-2 text-[10px] text-[#083536]/50 uppercase font-black tracking-widest">
                    <Clock size={12} />
            ...
            <p className="text-[#083536]/100 text-sm leading-relaxed mb-6 italic">
  ...
  return (
    <main className="max-w-4xl mx-auto p-4 sm:p-8 pb-32">
      ...
      <header className="mb-10">
        <h1 className="text-3xl font-bold text-[#083536] mb-2 flex items-center gap-3">
          <MessageSquare className="text-[#97709B]" /> Depoimentos
        </h1>
        <p className="text-[#083536]/80 text-sm">Controle as mensagens de carinho que aparecem no rodapé do seu site.</p>
      </header>
        ...
        {confirmModal.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl text-center">
              ...
              <h2 className="text-2xl font-bold text-[#083536] mb-2">Excluir Depoimento?</h2>
              <p className="text-[#083536]/60 mb-8">Esta ação não pode ser desfeita e o depoimento sairá do site permanentemente.</p>
              <div className="flex gap-3">
                <button onClick={() => setConfirmModal({ show: false })} className="flex-1 py-3 font-bold text-[#083536]/40 hover:text-[#083536]">Cancelar</button>
